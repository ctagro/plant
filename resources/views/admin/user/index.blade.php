                      ...
                      <div class="col-md-12">
                          <div class="card">
                              <div class="card-header">
                                ...
                                Tipos de Cultura
                                <a class="float-right" href="{{url('user/create')}}">Cadastrar</a>
                              </div>
                          </div>
                      </div>
                  </div>
                  ...
                  <table id="example1" class="table table-sm table-bordered table-striped dataTable dtr-inline collapsed" role="grid" aria-describedby="example1_info">
                      <thead>
                          <tr>
                              ...
                              <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">Competencia</th>
                    ...
                    <tbody>
                      ...
                      @foreach($users as $user)
                        ...
                        <tr>
                          ...
                          <td>  
                          <a href= "{{ route('user.edit' ,[ 'user' => $user->id  ])}}" >{{ $user->email}}</a>
                          </td>
                          <td>  
                            <a href= "{{ route('user.edit' ,[ 'user' => $user->id  ])}}" >{{ $user->competence_id}}</a>
                          </td>
        
                         <td>
            
                            <form id="delete-form"  method="POST" action="{{ route('user.destroy' ,[ 'user' => $user->id ])}}", style = 'display: inline;'> 
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}                 
                            </form>
                          </td>
